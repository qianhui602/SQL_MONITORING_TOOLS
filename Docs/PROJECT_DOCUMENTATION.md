...
#### 1.3.2 SQL 断联监控
- **连接状态检测**：每次采集周期主动 ping SQL Server 连接是否存活
- **状态记录**：在 `monitored_instances` 表中记录 `is_connected`、`last_connected_at`、`last_disconnected_at`、`connection_error` 字段
- **自动告警**：连接断开时触发 `connection_lost` 严重告警，恢复时触发 `connection_recovered` 恢复通知
- **前端展示**：Dashboard 页面显示所有实例的在线/离线状态，实例管理页面展示详细连接信息
...
#### 1.3.5 告警管理
- **内置规则**：内存使用率过高、死锁检测、采集任务中断、连接断开/恢复等
...
- **独立配置**：每个实例可独立配置连接信息
- **连接测试**：支持测试实例连接是否正常
- **连接状态展示**：实时显示每个实例的在线/离线状态、最后连接时间、错误信息
...
#### 1.3.12 帮助中心
- **帮助页面**：所有用户可访问的帮助中心页面（/help）
- **目录导航**：左侧目录导航，支持滚动高亮当前章节
- **内容搜索**：顶部搜索框实时过滤帮助内容
- **模块说明**：涵盖所有功能模块的使用说明和操作指引
- **常见问题**：包含 FAQ 和联系信息
...
#### 1.3.16 版本检测与升级提醒
- **版本检测**：自动对比本地版本与 GitHub 最新版本
- **升级提示**：侧边栏版本号显示黄色圆点提示新版本
- **升级横幅**：底部弹出升级通知，提供查看升级指南链接
- **升级指南**：README 提供 Docker 和本地开发两种升级步骤，含一键升级脚本
...
### 2.1 仪表盘 (Dashboard)
仪表盘是系统的首页，提供以下功能：
- **关键指标卡片**：CPU 使用率、内存使用量、活跃连接数、缓存命中率、磁盘使用率、批处理/秒、锁等待、死锁事件、数据库实例数（共 9 个）
- **卡片自定义**：支持拖拽排序和显示/隐藏统计卡片
- **数据库连接状态**：实时展示所有实例的在线/离线状态，带呼吸灯动画指示
...
| is_connected | Boolean | 当前连接状态：True=在线，False=离线 |
| last_connected_at | DateTime | 最后成功连接时间 |
| last_disconnected_at | DateTime | 最后断开时间 |
| connection_error | String(500) | 最近的连接错误信息 |
