...
### 告警与通知
- **告警系统**：自定义告警规则，支持多渠道组合通知
- **飞书应用通知**：关键错误（critical）通过飞书自建应用直达指定用户，接收人支持 open_id 或邮箱地址（自动识别 receive_id_type），失败时透出具体原因
- **群机器人 Webhook**：支持钉钉 / 企业微信 / 飞书群机器人推送
- **邮件通知**：通过 SMTP 发送告警邮件
- **渠道弹窗配置与测试**：系统设置中各通知渠道以卡片展示，点击弹出配置窗口并一键发送测试消息
- **声音提醒**：新告警声音通知，支持静音开关
...
- **通知渠道配置**：管理员在"系统设置 → 通知服务"中以卡片方式配置邮件、钉钉、企业微信、飞书渠道，点击卡片弹出配置窗口，可发送测试消息验证连通性；飞书应用通知接收人支持邮箱地址或 open_id
