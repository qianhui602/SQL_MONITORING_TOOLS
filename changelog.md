# Changelog

## 2026-08-26

**标题：** 关键错误飞书应用通知 / 接收人邮箱支持 / 通知渠道配置弹窗化 / 通知开关即时生效
**文件：** `backend/app/services/notification.py`, `backend/app/routers/feishu_test.py`, `backend/app/routers/smtp_test.py`, `backend/app/init_db.py`, `frontend/src/views/Settings.vue`, `frontend/src/components/Layout.vue`, `frontend/src/i18n/zh-CN.js`, `frontend/src/i18n/en-US.js`
**明细：**
- 新增关键错误飞书应用通知（FeishuAppNotifier）：通过飞书自建应用调用 tenant_access_token 与 im/v1/messages 接口，直接向指定用户推送红色交互卡片告警，区别于群机器人 Webhook 渠道
- 仅 severity 为 critical 的严重告警触发飞书应用通知；tenant_access_token 类级别缓存（有效期约 2 小时，提前 5 分钟刷新）
- 接收人同时支持 open_id 和邮箱地址：值包含 @ 时自动识别为 email 类型的 receive_id_type，否则使用 open_id
- 发送结果改为返回 (是否成功, 错误描述) 元组，细分网络连接失败、请求超时、HTTP 状态错误等异常并将具体原因透出到前端提示
- 飞书配置优先读取数据库 system_configs（feishu_app_enabled / feishu_app_id / feishu_app_secret / feishu_receive_open_id），回退环境变量 FEISHU_APP_ID 等
- 设置页"通知服务"改版：企业微信 / 飞书应用 / SMTP 邮件 / 飞书 Webhook 四个渠道以卡片网格展示，点击卡片弹出配置窗口，支持发送测试消息并显示具体结果
- 侧边栏布局修复：layout 容器改为 min-height: 100vh 允许页面滚动，sidebar 采用 position: sticky 固定，无需缩小浏览器即可完整查看所有菜单

---

## 2026-07-09

**标题：** 标签页去图标 / 帮助页面扩充 / 浏览器桌面通知
**文件：** `frontend/src/components/Layout.vue`, `frontend/src/views/Help.vue`
**明细：**
- 顶部多标签页移除图标，仅显示文字，视觉更简洁
- 标签栏 padding 微调优化间距
- 帮助页面 16 个章节内容全面扩充，包含功能说明、操作步骤、指标解释、常见问题等
- 新增浏览器桌面通知功能（基于 Notification API）
- 通知面板底部新增"声音提醒"和"桌面通知"两个开关
- 新告警触发时推送系统级桌面通知，点击跳转到告警页面
- 桌面通知支持权限检测、状态持久化、浏览器不支持时自动隐藏

---

## 2026-07-09T15:30:00.000Z

**标题：** 修复 /help 路由未注册导致显示未知页面
**文件：** `frontend/src/router/index.js`
**明细：**
- 侧边栏菜单已添加帮助入口，但路由表中未注册 /help 路由
- 在 Vue Router 中新增 `/help` 路由，指向 Help.vue 组件
- 用户可正常访问帮助页面，不再提示"未知页面"

---

## 2026-07-09

**标题：** 多标签页导航栏
**文件：** `frontend/src/components/Layout.vue`
**明细：**
- 新增多标签页导航栏（参考 Dify/圣农渠道等产品风格）
- 导航到新页面时自动创建标签，支持点击切换
- 标签支持关闭（X 按钮），首页标签不可关闭
- 右键菜单支持关闭当前、关闭其他、关闭全部
- 工具栏右侧添加一键关闭其他标签按钮
- 当前激活标签自动滚动到可视区域
- 标签样式支持深色模式

---

## 2026-07-09

**标题：** 帮助中心页面
**文件：** `frontend/src/views/Help.vue`, `frontend/src/router/index.js`, `frontend/src/components/Layout.vue`
**明细：**
- 新增帮助中心页面（/help），所有用户可访问
- 左侧目录导航 + 右侧内容展示，支持滚动高亮
- 顶部搜索框实时过滤帮助内容
- 涵盖所有模块使用说明（总览、性能趋势、死锁、告警、慢查询等）
- 包含常见问题 FAQ 和联系我们
- 导航菜单新增帮助入口

---

## 2026-07-09

**标题：** 移除告警确认按钮
**文件：** `frontend/src/views/Alerts.vue`
**明细：**
- 移除告警管理页面的"确认状态"列和"操作"列
- 移除确认按钮和已处理状态文字
- 清理相关 CSS 样式和未使用的 API 导入

---

## 2026-07-09

**标题：** 版本检测与升级提醒功能
**文件：** `backend/app/routers/version.py`, `frontend/src/components/Layout.vue`, `frontend/src/api/index.js`, `README.md`
**明细：**
- 新增后端版本检查 API（/api/version/check），对比本地与 GitHub 最新版本
- 修复 main.py 版本号硬编码问题，统一使用 config.py 的 VERSION
- 侧边栏底部版本号支持动态获取，有新版本时显示黄色闪烁圆点
- 底部弹出升级通知横幅，包含版本信息和升级指南链接
- README 新增完整升级指南（Docker 和本地开发两种方式）
- 提供 upgrade.sh 和 upgrade.bat 一键升级脚本

---

## 2026-07-09

**标题：** 登录页面 UI 优化
**文件：** `frontend/src/views/Login.vue`
**明细：**
- 左侧品牌区新增渐变背景和浮动装饰圆圈动画
- 添加网格纹理背景增强科技感
- Logo 增加轻微浮动动画效果
- 功能特性卡片增加玻璃拟态样式和悬停动画
- 右侧登录卡片采用玻璃拟态设计 + 顶部渐变装饰条
- 输入框优化为粗边框、圆角、焦点发光效果
- 登录按钮采用渐变色 + 悬停动画
- 错误提示增加图标和圆角样式
- 页面加载时内容淡入和卡片滑入动画

---

## 2026-07-09

**标题：** 统计卡片显示/隐藏功能
**文件：** `frontend/src/views/Dashboard.vue`
**明细：**
- 统计卡片支持自定义显示/隐藏（9个指标均可独立控制）
- 自定义面板新增"统计卡片"分类区域
- 统计卡片与图表使用同一自定义面板，分段显示
- 显示状态持久化到 localStorage，刷新页面后保持设置
- 添加面板分隔线样式，区分统计卡片和图表区域

---

## 2026-07-01

**标题：** Dashboard 页面显示样式优化
**文件：** `frontend/src/views/Dashboard.vue`
**明细：**
- 统计卡片新增彩色渐变图标（9种指标对应不同颜色和图标）
- 统计卡片顶部增加渐变装饰条，悬停时显示
- 统计卡片悬停效果优化：上浮 + 阴影增强 + 图标放大
- 数据库连接状态区域头部增加图标和渐变背景
- 在线状态指示器增加呼吸灯脉动动画效果
- 实例状态标签改为带背景色的标签样式
- 列表项悬停时左侧显示蓝色指示条
- 图表卡片增加彩色圆点标识 + 顶部渐变条
- 图表放大按钮优化为图标按钮，悬停显示背景
- 工具栏时间范围按钮改为分段式胶囊设计
- 所有下拉框、按钮统一圆角和焦点状态样式
- 自定义面板、排序面板增加淡入动画
- 模态框增加缩放滑入动画 + 背景毛玻璃效果
- Loading 覆盖层增加毛玻璃模糊效果
- 完整适配深色模式（图标背景、状态标签等）
- 优化多断点响应式布局（1200px/900px/768px/640px/480px）

---
