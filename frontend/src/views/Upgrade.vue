  ...
  <div class="upgrade-page">
    ...
    <div class="card sys-info-card">
      <div class="card-header">
        ...
        <h3>系统信息</h3>
        <div v-if="loadingVersion || loadingGit" class="badge badge-loading">检测中...</div>
        <div v-else-if="versionData.has_update" class="badge badge-update">有新版本</div>
        <div v-else-if="versionData.error" class="badge badge-warn">版本检测异常</div>
        <div v-else class="badge badge-ok">正常</div>
      </div>

      <div v-if="loadingVersion || loadingGit" class="card-body">
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="upgrade-tabs">
          <button class="upgrade-tab" :class="{ active: upgradeMode === 'online' }" @click="upgradeMode = 'online'">在线升级</button>
          <button class="upgrade-tab" :class="{ active: upgradeMode === 'zip' }" @click="upgradeMode = 'zip'">ZIP 上传升级</button>
        </div>
        ...
        <div v-if="upgradeMode === 'online'">
          <div class="upgrade-steps">
            <div class="step" :class="stepState(0)">
              ...
              <div class="step-content"><div class="step-title">拉取代码</div></div>
            </div>
            ...
            <div class="step" :class="stepState(1)">
              ...
              <div class="step-content"><div class="step-title">更新文件</div></div>
            </div>
...
const versionData = ref({ current_version: '0.0.0', latest_version: '', has_update: false, release_url: '', release_notes: '', error: '', upgrade_enabled: true })

const gitStatus = ref({ is_git_repo: false, current_version: '0.0.0', project_ready: false, has_backend: false, has_frontend: false, has_docker_compose: false, hint: '' })
...
async function onCheckVersion() {
  checking.value = true
  try { versionData.value = await checkUpgrade() }
  catch (e) { versionData.value = { current_version: '0.0.0', latest_version: '', has_update: false, error: e.message } }
  finally { checking.value = false; loadingVersion.value = false }
}

async function onCheckGitStatus() {
  checkingGit.value = true
  try { gitStatus.value = await getUpgradeGitStatus() }
  catch (e) { gitStatus.value = { is_git_repo: false, error: e.message } }
  finally { checkingGit.value = false; loadingGit.value = false }
}

async function onApplyUpgrade() {
  if (!confirm('确定要执行升级吗？')) return
  upgrading.value = true; upgradeStep.value = 0; upgradeSuccess.value = null; upgradeLogs.value = []
  try {
  ...
  } catch (e) { upgradeSuccess.value = false; upgradeLogs.value.push(`[错误] ${e.message}`) }
  finally {
    upgrading.value = false
    nextTick(() => { if (logRef.value) logRef.value.scrollTop = logRef.value.scrollHeight })
    onCheckVersion(); onCheckGitStatus()
  }
}

onCheckVersion(); onCheckGitStatus()

function triggerFileInput() { if (!zipUploading.value) zipFileInput.value.click() }
function onZipFileSelect(e) { const f = e.target.files[0]; if (f && f.name.endsWith('.zip')) zipFile.value = f }
function onDropZip(e) { isDragging.value = false; const f = e.dataTransfer.files[0]; if (f && f.name.endsWith('.zip')) zipFile.value = f }
function formatFileSize(b) { if (!b) return '0 B'; const u = ['B','KB','MB','GB']; let i=0,s=b; while(s>=1024&&i<u.length-1){s/=1024;i++} return `${s.toFixed(1)} ${u[i]}` }

async function onUploadZip() {
  if (!zipFile.value) return
  if (!confirm('确定要上传 ZIP 文件进行升级吗？')) return
  zipUploading.value = true; zipSuccess.value = null; zipLogs.value = []
  try {
  ...
  } catch (e) { zipSuccess.value = false; zipLogs.value.push(`[错误] ${e.message}`) }
  finally {
...
[data-theme='dark'] .sys-item { background: #0f172a; }
</style>
